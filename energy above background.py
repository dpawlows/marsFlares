# -*- coding: utf-8 -*-
"""
Created on Sun Jun  9 15:26:40 2024
...
def process_cdf(file_url, retries=3, delay=5):
        ...
        try:
            ...
            try:
                with CDF(temp_file_path) as cdf:
                    ...
                    # Find the indices of the flare event
                    flare_start_idx = np.where(filtered_time == np.min(filtered_time))[0][0]
                    flare_end_idx = np.where(filtered_time == np.max(filtered_time))[0][0]

                    # Fit a linear regression line to the data before and after the flare event
                    slope, intercept, _, _, _ = linregress(filtered_time[flare_start_idx:flare_end_idx], 
                                                           filtered_data_diode_a[flare_start_idx:flare_end_idx])

                    # Compute the background line
                    background_line = slope * filtered_time[flare_start_idx:flare_end_idx] + intercept

                    # Compute the area above the background line during the flare event
                    area_above_background = np.trapz(filtered_data_diode_a[flare_start_idx:flare_end_idx] - background_line, 
                                                      filtered_time[flare_start_idx:flare_end_idx])
            ...
            if attempt < retries - 1:
                ...
                print(f"Failed to process {file_url} after {retries} attempts.")
                return None
...
# Check if matched_dates is not empty
if not matched_dates:
    print("No matched dates found. Exiting.")
else:
    results = []

    output_file = 'energy_results_above_background.csv'
    with open(output_file, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Date', 'File', 'Area Trapz Diode A', 'Area Simps Diode A', 'Area Trapz Diode C', 'Area Simps Diode C', 'Background Slope', 'Background Intercept', 'Area Above Background'])
        ...
        for date, file_url in matched_dates:
            ...
            if result is not None:
                ...
                writer.writerow([
                    ...
                    result['area_trapz_c'], 
                    result['area_simps_c'],
                    result['background_slope'],
                    result['background_intercept'],
                    result['area_above_background']
                ])
